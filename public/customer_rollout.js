// script_customer_rollout.js (CUSTOMER)
...
(function () {
  ...
  async function run() {
    ...
    console.log("[ROLLOUT CUSTOMER] payload:", payload);
    ...
    try {
      ...
      console.log("[ROLLOUT CUSTOMER] response:", est);
  ...
  // Support both: onclick handlers and a button
  window.submitCustomerRolloutEstimate = async function (e) {
    try { e && e.preventDefault && e.preventDefault(); } catch {}
    await run();
  };
  window.submitRolloutEstimate = window.submitRolloutEstimate || window.submitCustomerRolloutEstimate;

  document.addEventListener("DOMContentLoaded", () => {
    document.getElementById("btnCalc")?.addEventListener("click", (e) => { e.preventDefault(); run(); });
    document.getElementById("customerBtnCalc")?.addEventListener("click", (e) => { e.preventDefault(); run(); });
  });
})();
