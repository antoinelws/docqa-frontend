// Main Script
    (function () {
      ...
      function computeInternalEstimate(payload) {
        ...
        // 2) Already use this carrier with ShipERP?
        ...
        // 4) Z-Enhancements with this carrier
        ...
        // 10) Shipping FROM location (add 8 hours if not US)
        const shipFrom = Array.isArray(payload.shipFromLocation) ? payload.shipFromLocation : [];
        const hasNonUSFrom = shipFrom.some(loc => loc !== "US" && loc.trim() !== "");
        if (hasNonUSFrom) {
        ...
        // 11) Shipping TO location (add 8 hours if not US)
        const shipTo = Array.isArray(payload.shipToLocation) ? payload.shipToLocation : [];
        const hasNonUSTo = shipTo.some(loc => loc !== "US" && loc.trim() !== "");
        if (hasNonUSTo) {
